#!/usr/bin/env node
/**
 * Find _posts that are encrypted (encrypted: true and have payload),
 * then call node decrypt.js for each with the password.
 *
 * Password: 1st CLI arg, or .env (POST_PASSWORD=...), or POST_PASSWORD env, or prompt.
 * Usage: node scripts/decrypt_posts.js [password]
 */
...
const DECRYPT_SCRIPT = path.join(__dirname, 'decrypt.js');
...
async function main() {
  ...
  if (!password) {
    ...
    if (!password) {
      console.error('Password is required. Use: node scripts/decrypt_posts.js <password> or set POST_PASSWORD in .env.');
      process.exit(1);
    }
  }
  ...
  for (const file of files) {
    ...
    toDecrypt.push(filePath);
  }

  if (toDecrypt.length === 0) {
    console.log('No posts need decryption.');
    return;
  }

  const env = { ...process.env, POST_PASSWORD: password };

  for (const filePath of toDecrypt) {
    const result = spawnSync(process.execPath, [DECRYPT_SCRIPT, filePath], { env, stdio: 'inherit' });
    if (result.status !== 0) {
      process.exit(1);
    }
  }
